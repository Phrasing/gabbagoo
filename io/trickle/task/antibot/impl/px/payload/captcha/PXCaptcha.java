 ...
 *  io.trickle.task.antibot.impl.px.payload.captcha.PXCaptcha$1
...
lbl29:
    ...
    public CompletableFuture setBrowserType() {
        ...
        if (this.mobile) {
            ...
            deviceImpl = (Devices.DeviceImpl)completableFuture.join();
        } else {
    ...
    public static void main(String[] stringArray) {
        ...
        RealClient realClient = RealClientFactory.buildProxied((Vertx)vertx, (ClientType)ClientType.CHROME);
        ...
        switch (var8_9) {
            case 0: {
                ...
                while (var0.client.isActive() != false) {
                    try {
                        v0 = Request.send((HttpRequest)var2_2, (Buffer)var5_5);
                        if (!v0.isDone()) {
                            var7_8 = v0;
...
lbl22:
                        ...
                        if (!var0.client.isActive()) return CompletableFuture.failedFuture(new Exception("Failed to send payload. Active? " + var0.client.isActive()));
    ...
    public String getSiteID() {
        switch (1.$SwitchMap$io$trickle$task$sites$Site[this.SITE.ordinal()]) {
            case 1: {
                switch (1.$SwitchMap$io$trickle$task$antibot$impl$px$Types[this.type.ordinal()]) {
                    case 1: {
    ...
    public PXCaptcha(Logger logger, RealClient realClient, Types types, String string, String string2, Site site) {
        this.logger = logger;
        this.client = RealClientFactory.fromOther((Vertx)Vertx.currentContext().owner(), (RealClient)realClient, (ClientType)ClientType.CHROME);
        this.type = types;
    ...
    public CompletableFuture solveCaptcha(String string) {
        ...
        CompletableFuture completableFuture = VertxUtil.hardCodedSleep((long)l);
        if (!completableFuture.isDone()) {
            CompletableFuture completableFuture2 = completableFuture;
    ...
    public HttpRequest hibbettCSS() {
        ...
        httpRequest.putHeader("referer", this.origin + "/");
        httpRequest.putHeader("accept-encoding", this.device.getAcceptEncoding() == null ? "gzip, deflate" : DeviceHeaderParsers.getAcceptLanguage((JsonArray)this.device.getArr("PX313")));
        httpRequest.putHeader("accept-language", this.device.getAcceptLanguage());
        return httpRequest;
    }
    ...
    public CompletableFuture sendPayload(ExtendedPayload extendedPayload) {
        ...
        while (this.client.isActive()) {
            try {
                ...
                if (!this.client.isActive()) return CompletableFuture.failedFuture(new Exception("Failed to send payload. Active? " + this.client.isActive()));
